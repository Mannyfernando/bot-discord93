...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    let Config = new MessageEmbed()
      .setAuthor("Configuration du serveur", client.botconfig.IconURL)
...
`);
    ...
    if (em._emoji.name === "1️⃣") {
      await client.sendTime(message.channel, "En quoi voulez-vous changer le prefix ?");
      let prefix = await message.channel.awaitMessages(
        (msg) => msg.author.id === message.author.id,
        { max: 1, time: 30000, errors: ["time"] }
      );
      if (!prefix.first())
        return client.sendTime(message.channel, "Vous avez mis trop de temps à répondre.");
      prefix = prefix.first();
      ...
      client.sendTime(
        message.channel, `Prefix du serveur enregistré avec succès en tant que \`${prefix}\``
      );
    } else {
      await client.sendTime(
        message.channel, "Veuillez mentionner le rôle que vous souhaitez que les « DJ » jouent."
      );
      ...
      client.sendTime(
        message.channel, "Rôle DJ enregistré avec succès en tant que <@&" + role.id + ">"
      );
    }
  },
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (config === "prefix") {
        ...
        if (
          ...
          client.sendTime(interaction, `Le prefix est maintenant défini sur \`${prefix}\``);
        } else {
          //has not prefix
          client.sendTime(interaction, `Le prefix de ce serveur est \`${GuildDB.prefix}\``);
        }
