...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Usage - **\`${GuildDB.prefix}search [mettre en doute]\``
      );
        ...
        switch (res.loadType) {
          ...
          case "PLAYLIST_LOADED":
            ...
            if (
              ...
              player.play();
            return client.sendTime(
              interaction, `**Playlist ajoutée à la file d'attente**: \n**${res.playlist.name}** \nEn file d'attente : **${res.playlistInfo.length} sons**`
            );
          case "SEARCH_RESULT":
            let max = 10,
              collected,
            ...
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `Le nombre que vous avez fourni était supérieur ou inférieur au total de la recherche. Utilisation -\`(1-${max})\``
              );
