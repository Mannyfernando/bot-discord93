...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!player.queue || !player.queue.length || player.queue.length === 0) return client.sendTime(message.channel, "❌ | **Pas assez de chansons dans la file d'attente à mélanger !**");
     ...
     */
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (!player.queue || !player.queue.length || player.queue.length === 0) return client.sendTime(interaction, "❌ | **Pas assez de chansons dans la file d'attente à mélanger !**");
            player.queue.shuffle();
            client.sendTime(interaction, "✅ | Mélangez la file d'attente !");
        },
    },
};
