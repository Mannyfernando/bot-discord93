...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    try {
      ...
      return client.sendTime(message.channel, `⏭ Ignoré \`${Number(args[0] - 1)}\` sons`);
    } catch (e) {
      console.log(String(e.stack).bgRed);
      client.sendError(message.channel, "Quelque chose s'est mal passé.");
    }
  },
  SlashCommand: {
    options: [
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      try {
        if (!interaction.data.options) return client.sendTime(interaction, `**Usage**: \`${GuildDB.prefix}skipto <numéro>\``);
        ...
        player.stop(skipTo);
        //Send Success Message
        return client.sendTime(interaction, `⏭ Ignoré \`${Number(skipTo)}\` sons`);
      } catch (e) {
        console.log(String(e.stack).bgRed);
