...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!SearchString) return client.sendTime(message.channel, `** - **\`${GuildDB.prefix}play [son]\``);
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.setAuthor(`Playlist ajoutée à la file d'attente`, message.author.displayAvatarURL());
                    ...
                    return client.sendTime(message.channel, "**Aucune correspondance trouvée pour - **" + SearchString);
                }
            } else {
                let Searched = await player.search(SearchString, message.author);
                if (!player) return client.sendTime(message.channel, "❌ | **Rien ne joue en ce moment...**");

                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "****" + SearchString);
                    ...
                    SongAddedEmbed.addField("En file d'attente", `\`${Searched.tracks.length}\` songs`, false);
                    SongAddedEmbed.addField("Durée de la liste de lecture", `\`${prettyMilliseconds(Searched.playlist.duration, { colonNotation: true })}\``, false);
                    Searching.edit(SongAddedEmbed);
                } else {
    ...
    SlashCommand: {
        ...
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (!member.voice.channel) return client.sendTime(interaction, "❌ | **Vous devez être dans un canal vocal pour jouer quelque chose !**");
            ...
            if (search.match(client.Lavasfy.spotifyPattern)) {
                ...
                switch (Searched.loadType) {
                    case "LOAD_FAILED":
                        ...
                        return client.sendError(interaction, `❌ | **Une erreur s'est produite lors de la recherche**`);

                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
                        return client.sendTime(interaction, "❌ | **Aucun resultat n'a été trouvé.**");
                    case "TRACK_LOADED":
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        Playlist.setAuthor(`Playlist ajoutée à la file d'attente`, client.botconfig.IconURL);
                        Playlist.setDescription(`[${Searched.playlistInfo.name}](${interaction.data.options[0].value})`);
                        Playlist.addField("En file d'attente", `\`${Searched.tracks.length}\` sons`, false);
                ...
                try {
                    ...
                    if (res.loadType === "LOAD_FAILED") {
                        if (!player.queue.current) player.destroy();
                        return client.sendError(interaction, `:x: | **Une erreur s'est produite lors de la recherche**`);
                    }
                } catch (err) {
                    return client.sendError(interaction, `Une erreur s'est produite lors de la recherche : ${err.message}`);
                }
                switch (res.loadType) {
                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        await player.play();
                        let SongAdded = new MessageEmbed();
                        SongAdded.setAuthor(`Playlist ajoutée à la file d'attente`, client.botconfig.IconURL);
