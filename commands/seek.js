...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!player.queue.current.isSeekable) return client.sendTime(message.channel, "❌ | **Je ne peux pas chercher cette chanson !**");
        let SeekTo = client.ParseHumanTime(args.join(" "));
        if (!SeekTo) return client.sendTime(message.channel, `**Utilisation - **\`${GuildDB.prefix}seek <nombre s/m/h>\` \n**Exemple - **\`${GuildDB.prefix}seek 2m 10s\``);
