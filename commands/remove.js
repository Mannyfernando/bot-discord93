...
const { TrackUtils } = require("erela.js");
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    let rm = new MessageEmbed()
      .setDescription(`✅ **|** Piste supprimée**\`${Number(args[0])}\`** from the queue!`)
      .setColor("GREEN")
      if (isNaN(args[0]))rm.setDescription(`**Utilisation -**${client.botconfig.prefix}\`remove [piste]\``);
      if (args[0] > player.queue.length)
      rm.setDescription(`La file d'attente n'a que ${player.queue.length} sons!`);
   ...
   */
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      const song = player.queue.slice(args[0] - 1, 1);
      if (!player) return client.sendTime(interaction, "❌ | **Rien ne joue en ce moment...**");
      ...
      if (!player.queue || !player.queue.length || player.queue.length === 0)
      return client.sendTime("❌ | **Rien ne joue en ce moment...**");
      let rm = new MessageEmbed()
        .setDescription(`✅ | **Piste supprimée** \`${Number(args[0])}\` de la file d'attente !`)
        .setColor("GREEN")
      if (isNaN(args[0])) rm.setDescription(`**Usage:** \`${GuildDB.prefix}remove [piste]\``);
      if (args[0] > player.queue.length)
        rm.setDescription(`La file d'attente n'a que ${player.queue.length} sons!`);
